...
esp_err_t ADXL345_init()
{
	i2c_cmd_handle_t cmd = i2c_cmd_link_create();
		esp_err_t error = 0;
		i2c_master_start(cmd);
		i2c_master_write_byte(cmd, 0xA6, 1 /* expect ack */);
		i2c_master_write_byte(cmd, 0x2D, 1 /* expect ack */);
		i2c_master_write_byte(cmd, 0x08, 1 /* expect ack */);
...
esp_err_t Get_MMA7660_XYZ(double * XYZ)
{
	...
	sphere[1] = sphere[1] + AccelOffSet[1];
